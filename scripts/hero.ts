...
function generateNodes() {
  ...
  for(let i=0; i<points.length; i++) {
    for(let j=i+1; j<points.length; j++) {
      ...
      if(dist < 150) {
        lines += `<line x1="${points[i].x}" y1="${points[i].y}" x2="${points[j].x}" y2="${points[j].y}" stroke="${COLORS.primary}" stroke-width="0.5" stroke-opacity="${1 - dist/150}" />`;
      }
    }
  }
  
  for(let i=0; i<points.length; i++) {
    nodes += `<circle cx="${points[i].x}" cy="${points[i].y}" r="2" fill="${COLORS.accent}" fill-opacity="0.6">
...
const heroContent = `
  ...
  <g transform="translate(50, 80)">
    ...
    <circle cx="20" cy="20" r="6" fill="${COLORS.error}" />
    <circle cx="40" cy="20" r="6" fill="${COLORS.warning}" />
    <circle cx="60" cy="20" r="6" fill="${COLORS.success}" />
    <line x1="0" y1="40" x2="350" y2="40" stroke="${COLORS.border}" stroke-width="1" />
    ...
    <g class="mono terminal-text" transform="translate(20, 70)">
      ...
      <rect class="cursor line5" x="110" y="88" width="8" height="15" fill="${COLORS.success}" />
    </g>
  </g>
  ...
  <g transform="translate(450, 140)" class="line-final">
    ...
    <text class="subtitle" y="70" font-size="14" fill="${COLORS.accent}">Building the future with AI.</text>
  </g>
`;

const svg = wrapSVG(heroContent, width, height, customStyles);
writeSVG('assets/animations/hero.svg', svg);
